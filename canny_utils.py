...
def gradient_x(img):
    grad_img = ndimage.convolve(img, np.array([[-1, 0, 1], [-SOBEL_CENTER_PARAM, 0, SOBEL_CENTER_PARAM], [-1, 0, 1]]))
    return grad_img/np.max(grad_img)

def gradient_y(img) :
    grad_img = ndimage.convolve(img, np.array([[-1, -SOBEL_CENTER_PARAM, -1], [0, 0, 0], [1, SOBEL_CENTER_PARAM, 1]]))
    return grad_img/np.max(grad_img)
...
def hysteresis_thresholding(img,low_ratio=0.03, high_ratio=0.05):
    # low_ratio = 0.03
    # high_ratio = 0.05
